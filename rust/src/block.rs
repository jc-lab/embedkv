...
/// 32-byte storage header stored in block 0.
...
pub fn marshal_storage_header(h: &StorageHeader, buf: &mut [u8]) {
    ...
    buf[28..32].copy_from_slice(&h.block_crc32.to_le_bytes());
}

/// Deserializes the storage header and verifies its CRC32.
...
pub fn unmarshal_storage_header(buf: &[u8], h: &mut StorageHeader) -> bool {
    ...
    h.block_crc32 = u32::from_le_bytes([buf[28], buf[29], buf[30], buf[31]]);
    compute_block_crc32(buf, 28) == h.block_crc32
}
...
/// Deserializes the descriptor header and verifies its CRC32.
/// Returns `true` only when the CRC is valid.
...
/// Fixed 24-byte header of a value chunk block.
#[derive(Debug, Clone, Default)]
...
pub fn marshal_chunk_header(h: &ValueChunkHeader, buf: &mut [u8]) {
    ...
    buf[20..24].copy_from_slice(&h.block_crc32.to_le_bytes());
}

/// Deserializes the chunk header and verifies its CRC32.
/// Returns `true` only when the CRC is valid.
pub fn unmarshal_chunk_header(buf: &[u8], h: &mut ValueChunkHeader) -> bool {
    ...
    h.block_crc32 = u32::from_le_bytes([buf[20], buf[21], buf[22], buf[23]]);
    compute_block_crc32(buf, 20) == h.block_crc32
}
